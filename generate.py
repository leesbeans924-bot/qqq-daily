...
MACRO = ["^TNX", "^VIX", "CL=F"]   # 10Y yield, VIX, crude oil
    ...
    for ticker in tickers:
        try:
            ...
            if len(hist) >= 2:
                ...
                data[ticker] = {"close": round(close, 2), "change": round(chg, 2), "prev": round(prev, 2)}
...
def fetch_qqq_30d():
    ...
    try:
        ...
        return [{"date": d.strftime("%b %d"), "close": round(c, 2)}
...
def get_analysis(quotes, macro, chart_data):
    ...
    holdings_text = "\n".join(
        f"  {MACRO_LABELS.get(t, QQQ_HOLDINGS.get(t, t))}: "
        f"${q['close']} ({'+' if q['change']>0 else ''}{q['change']}%)"
        for t, q in quotes.items()
    )

    macro_text = "\n".join(
        f"  {MACRO_LABELS.get(t, t)}: {q['close']} ({'+' if q['change']>0 else ''}{q['change']}%)"
...
30-DAY QQQ PRICE TRAIL (oldest → newest): {[d['close'] for d in chart_data[-10:]]}
...
3. CHART PATTERN (2-3 sentences analyzing the 30-day price action. Name the pattern if there is one — e.g. bull flag, double top, ascending triangle. Be direct about what the technicals say.)
...
def build_html(quotes, macro, chart_data, analysis):
    ...
    macro_html = "".join(macro_row(t) for t in MACRO)

    # Chart data JSON
    chart_json = json.dumps(chart_data)

    # Format analysis sections as HTML
...
<head>
  ...
  <style>
    ...
    * {{ box-sizing: border-box; margin: 0; padding: 0; }}
    ...
    .date {{ font-size: 12px; color: var(--muted); letter-spacing: 0.08em; text-transform: uppercase; }}
    ...
    @media (max-width: 700px) {{
      ...
      .panel, .panel-full {{ padding: 20px; }}
      .headline-bar {{ padding: 14px 20px; }}
      .qqq-hero {{ font-size: 32px; }}
      .updated {{ padding: 14px 20px; flex-direction: column; gap: 4px; }}
    }}
  </style>
</head>
<body>
...
<div class="grid">
  ...
  <div class="panel">
    ...
    {macro_html}
    <br>
    <div class="panel-label" style="margin-top:16px">30-Day Chart</div>
  ...
  <div class="grid" style="background:transparent; border:none; gap:32px;">
    <div>
      <div class="analysis-section">
        <h3>What Moved QQQ Today</h3>
        {bullets_to_html(movers)}
      </div>
      <div class="analysis-section">
        <h3>Chart Pattern</h3>
        {para_to_html(pattern)}
      </div>
    </div>
    <div>
      <div class="analysis-section">
...
def main():
    ...
    macro  = fetch_quotes(macro_tickers)
    chart  = fetch_qqq_30d()
